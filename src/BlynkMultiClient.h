...
class BlynkArduinoClientMulti
    : public BlynkArduinoClientGen<T_Blynk>
{
    struct Entry {
        String    name;
        T_Blynk*        client;
...
public:
    ...
    void addClient(const String& name, T_Blynk& c, uint16_t port = BLYNK_DEFAULT_PORT_SSL) {
        if (_conn_qty == 0) {
            BlynkArduinoClientGen<T_Blynk>::setClient(&c);
            BlynkArduinoClientGen<T_Blynk>::port = port;
        }
    ...
    bool connect() {
        for (int i = 0; i < _conn_qty; i++) {
            BlynkArduinoClientGen<T_Blynk>::setClient(_conn_array[i].client);
            BlynkArduinoClientGen<T_Blynk>::port = _conn_array[i].port;
            if (BlynkArduinoClientGen<T_Blynk>::connect()) {
    ...
    void disconnect() {
        BlynkArduinoClientGen<T_Blynk>::isConn = false;
        for (int i = 0; i < _conn_qty; i++) {
...
public:
    ...
    template<typename T_Blynk>
    void addClient(const String& name, T_Blynk& client, uint16_t port = BLYNK_DEFAULT_PORT_SSL) {
